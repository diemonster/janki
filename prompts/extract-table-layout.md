...
This source is a list or table whose printed form columns the repository
owner has already bound, and its scope is the whole supplied document:
...
This page's printed form columns were bound by the repository owner before
this request and are listed in the request data under `Printed source-form
columns for this page`. Each entry gives one column's opaque `id`, its printed
position, the exact heading text the owner recorded for it, and the display
label they chose. That list is the complete set of form columns for this page;
it is not a suggestion and there is no expected count.

Copy each candidate row's printed cells into conjugations keyed by those exact
`id` values. Never use a printed heading, a display label, a translation of
either, or an id that the request did not list. A column whose cell that row
prints blank is an empty string under its id; a column that row has no cell for
gets no key at all. Copy a clipped, misprinted, or otherwise doubtful cell
exactly as printed and leave the same evidence verbatim in context; explain the
doubt in usage_notes and use low confidence rather than correcting it. Do not
substitute a familiar set of derived forms, do not reorder or merge the listed
columns, and do not omit a column whose cell simply repeats the candidate's
expression or reading. Return conjugations empty for a row that prints no cell
under any listed column.

Do not report which printed heading you matched to which id, and do not
describe the mapping in usage_notes or anywhere else: the owner already bound
it, and the ids in the request are the whole of that decision.
