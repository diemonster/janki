...
**S6-P (review and promotion) is complete at `972ef32`.** Its actual
signatures, wires and ownership exceptions are recorded in the
[S6 interface record §2.5–§2.8 and §4](ASSISTANT_STUDY_JOBS_S6_INTERFACES.md).
Independent review and root verification closed the aggregate-review,
archive-recovery, lock-order and unstarted-redecision defects. Final
...
it binds, so replacing and restoring a store during preparation cannot drop
saved entries. Failed refreshes disclose when saved facts were retained.
Two independent review rounds and root verification are complete. Focused
behavior tests and production mutations cover the new contracts; the four
new correction mutants and the one affected prior mutant were run without
repeating the other 29 unchanged mutation checks. Final `make gates` passed
**5830 tests**, with 800 warnings, Ruff clean and the sample deck built
(516.70s). The reviewed production code and test assertions remained
unchanged through that run.

The testing policy now requires focused nodes/files and directly affected
callers during changes and review fixes, preserved full output and direct exit
