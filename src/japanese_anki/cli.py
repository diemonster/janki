...
def command_audio(args: argparse.Namespace) -> int:
    """Run the shared durable audio service and format its CLI transcript."""
    config = _load_config(args)
    options = dict(
        words=args.words,
...
def _print_audio_outcome(
    outcome: audio_application.AudioExecutionOutcome, *, normalized_path: Path
) -> int:
    """Render the shared service result without reconstructing transaction truth."""
    for warning in outcome.warnings:
        print(f"warning: {warning}", file=sys.stderr)
    if outcome.guessed_accent:
        print(
            f"warning: {len(outcome.guessed_accent)} record(s) have no usable "
            ...
            f"{', '.join(outcome.guessed_accent[:5])}"
            + (" ..." if len(outcome.guessed_accent) > 5 else ""),
            file=sys.stderr,
        )
    if outcome.no_reading:
        print(
            f"warning: {len(outcome.no_reading)} record(s) have no reading to "
            f"speak: {', '.join(outcome.no_reading[:5])}"
            + (" ..." if len(outcome.no_reading) > 5 else ""),
            file=sys.stderr,
        )
    if outcome.state == "records-stale":
        print(f"error: {outcome.stopped_by}", file=sys.stderr)
        return 1
    if outcome.no_records:
        if outcome.prune_error:
            print(f"error: {outcome.prune_error}", file=sys.stderr)
            return 1
        print(f"No records to voice in {normalized_path}.")
        if outcome.prune_requested:
            print(f"Pruned {len(outcome.pruned_paths)} unreferenced clip(s).")
        return 0

    print(
        f"Wrote {outcome.file_count} clip(s) for "
        f"{len(outcome.written_record_ids)} record(s) into {outcome.output_dir}."
        + (f" {outcome.up_to_date} already current." if outcome.up_to_date else "")
    )
    if outcome.prune_requested and outcome.ledger_error is None:
        print(f"Pruned {len(outcome.pruned_paths)} unreferenced clip(s).")
    ...
    if outcome.ledger_error:
        print(f"warning: {outcome.ledger_error}", file=sys.stderr)
        print(
...
def _print_promotion_execution(result: PromotionExecutionResult) -> int:
    ...
    if result.state == "pattern_only":
        assert result.archive_path is not None
        print(f"Completed reviewed pattern-only extraction from {path}.")
        print("  No records were promoted.")
        print(
            f"  Archived to {result.archive_path}"
            + (" (exact retry)." if result.archive_was_retry else "")
        )
        return 0

    if result.state == "archive_retry":
        assert result.archive_path is not None
        if result.empty_live_retry:
            print(
            ...
            print(
                f"Completed exact archive retry for {path}; {result.removed} "
    ...
    if result.state == "landed_ai_ledger_incomplete":
        assert result.ledger_error is not None
        print(
            f"Promoted {len(result.promoted)} record(s) from {path} into "
            f"{result.output_path}; the live staging review was kept."
        )
        _print_merge_summary(dict(result.outcomes), prefer_incoming_available=False)
        print(_ledger_line(result.ledger_added, result.ledger_sources, written=False))
        _report_enrichment_ledger_failure(
            result.ledger_error,
            rerun=(
    ...
    assert result.state in ("landed", "landed_ledger_incomplete")
    assert result.archive_path is not None
    print(
        f"Promoted {len(result.promoted)} record(s) from {path} into "
    ...
    if result.held:
        ...
        print(
            f"  {path} is finished and was deleted "
    ...
    print(
        _ledger_line(
            ...
            written=result.ledger_error is None,
        )
    )
    if result.ledger_error is not None:
        _report_ledger_failure(result.ledger_error)
        return 1
    return 0
...
def _promote_under_guard(
    ...
    def offline() -> PromotionDecision:
        ...
        return decide_promotion(
            config,
            path,
            skip_reading_check=True if args.skip_reading_check else None,
        )
    ...
    if decision.state not in ("nothing", "pattern_only", "archive_retry"):
        for problem in decision.unreadable_decks:
            print(f"warning: {unreadable_deck_warning(problem)}", file=sys.stderr)
        for warning in decision.reading_warnings:
            print(f"warning: {warning}", file=sys.stderr)

    execution = promotion_application.execute_promotion(config, decision)
    return _print_promotion_execution(execution)
