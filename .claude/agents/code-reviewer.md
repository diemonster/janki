...
Verify before you report. Read the surrounding code and the call sites; a finding that dissolves on one more file read is worse than no finding. Use the evidence that already exists first — the tests themselves, the implementer's saved run output and mutation logs — rather than re-running work over unchanged bytes, which proves nothing the first run did not. When a finding genuinely needs execution, run the focused nodes or files that own the behaviour, plus any directly affected caller, and anchor every run to the checkout under review: its own `src` on `PYTHONPATH` and its venv Python running pytest as a module, never bare `pytest` or bare `janki`, which resolve through the venv's editable install to the primary worktree and will report green for code your branch never changed. Keep each run's complete stdout and stderr and the command's own exit status; a truncated log is evidence somebody has to produce again. Do not run `make gates`: the integration lead runs it once at the commit boundary, after the fixes are stable. Do not report pure style or naming preferences.
